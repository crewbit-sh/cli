...
```
- src/commands/status.ts — new file, the command
- src/cli.ts — route `status` to it, and name it in USAGE
```
...
so instead of listing them. Ten is a large diff in a package whose whole surface
is `src/`, `testkit/` and `tools/release/`.
...
"changing the existing batcher rather than adding a second one, because two
would each number their frames from zero and the server orders on that sequence"
invites a real objection. "Refactor for maintainability" invites nothing.
...
```
- [unit] a stop reason names the turn ceiling when the engine gave no subtype
- [unit] a batch replayed after a reconnect leaves one row per event
- [integration] a Job handed on while the runner was away is dropped without a completion
```
...
missing and what would satisfy it, the way `.claude/rules/ready_for_code.md`
describes, and stop. Do not plan around the gap.
