...
- a job no longer sees the developer account's connectors or MCP servers
...
- a rate-limit warning no longer schedules a Job as if it were blocked

## 0.8.1

### Fixed

- crewbit --help now lists run list
...
- crewbit run view can print one artifact a stage left behind
- answer the plan gate from the terminal, without opening a browser

### Fixed

- a stream of nothing but timing pings no longer eats a transcript's own room

## 0.6.0

### Breaking

- crewbit lists your projects and reads one, and every command takes a verb

### Added

- crewbit starts planning a spec from the command line
- crewbit lists the specs a project is offering
- crewbit lists your projects and reads one, and every command takes a verb
...
- crewbit run <id> reads a Run's state without opening a browser

## 0.4.0

### Added

- running the runner is `crewbit runner`, not `crewbit`
